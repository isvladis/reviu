...
| **018** _(2026-06-28)_ | **Rol `moderator` como columna `profiles.role` (enum `user_role`)** + `objects.rejection_reason` + policies de moderación. Helper `has_role()` como punto único de comprobación desde RLS. | Fase 1 necesita revisión humana antes de publicar (ADR-017). El rol vive como columna mientras no haya `role_grants` (Fase 2). Cuando llegue Fase 2, **solo cambia la implementación de `has_role()`** para consultar `role_grants` (caducable); las policies no se reescriben. Esto preserva ADR-007 (no permisos eternos) sin reescritura. Migración: `0004_moderation.sql`. | (a) Hardcodear emails de moderadores (no rota, no auditable). (b) Adelantar `role_grants` a Fase 1 (sobreingeniería para un único rol funcional). |
| **019** _(2026-06-29)_ | **Preferencias de contacto + GRANTs por columna en `profiles`**. Columnas nuevas: `phone`, `contact_email`, `contact_phone`, `contact_inapp`. A `anon` se le revoca el SELECT general y se reconcede SELECT **solo** sobre las columnas públicas (`id, display_name, neighborhood, city, avatar_url, bio`). `authenticated` conserva SELECT sobre todas las columnas; el servidor decide si revela `phone`/email del publicador según el consentimiento del propio dueño. El email vive en `auth.users` y se obtiene con `service_role` solo cuando el publicador ha dado `contact_email = true`. | Las policies RLS filtran filas, no columnas: bloquear lectura por columna requiere GRANTs por columna. Esta es la barrera estructural más simple y robusta para garantizar que el usuario anónimo **nunca** pueda leer teléfono o preferencias del publicador, incluso si la app servidora tuviera un fallo. Migración: `0005_profile_contact_prefs.sql`. | (a) Vista `profiles_public` (duplica esquema, complica tipos generados). (b) Función SECURITY DEFINER por cada consulta (sobreingeniería para Fase 1). (c) Confiar solo en filtrado en servidor sin GRANT por columna (rompe el principio "RLS lo garantiza en BD" de §3). |
